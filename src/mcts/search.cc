...
class MEvaluator {
 ...
 private:
  static bool WithinThreshold(const Node* parent, float q_threshold) {
    return std::abs(parent->GetQ(0.0f, 1.0f, 0.0f, 1.0f)) > q_threshold;
  }
...
void Search::SendUciInfo() REQUIRES(nodes_mutex_) REQUIRES(counters_mutex_) {
  ...
  int multipv = 0;
  const auto default_q = -root_node_->GetQ(-draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
  ...
  for (const auto& edge : edges) {
    ...
    const auto q = edge.GetQ(default_q, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
...
inline float GetFpu(const SearchParams& params, Node* node, bool is_root_node,
                    float draw_score, float frustration, float disappointment, float upset) {
  const auto value = params.GetFpuValue(is_root_node);
  return params.GetFpuAbsolute(is_root_node)
             ? value
             : -node->GetQ(-draw_score, frustration, disappointment, upset) -
                   value * std::sqrt(node->GetVisitedPolicy());
}

// Faster version for if visited_policy is readily available already.
inline float GetFpu(const SearchParams& params, Node* node, bool is_root_node,
                    float draw_score, float visited_pol, float frustration, float disappointment, float upset) {
  const auto value = params.GetFpuValue(is_root_node);
  return params.GetFpuAbsolute(is_root_node)
             ? value
             : -node->GetQ(-draw_score, frustration, disappointment, upset) - value * std::sqrt(visited_pol);
}
...
std::vector<std::string> Search::GetVerboseStats(Node* node) const {
  ...
  const float fpu = GetFpu(params_, node, is_root, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
  ...
  std::sort(edges.begin(), edges.end(),
            [&](EdgeAndNode a, EdgeAndNode b) {
              return std::forward_as_tuple(
                         a.GetN(), a.GetQ(fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset()) + a.GetU(U_coeff)) <
                     std::forward_as_tuple(
                         b.GetN(), b.GetQ(fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset()) + b.GetU(U_coeff));
            });
  ...
  auto print_stats = [&](auto* oss, const auto* n) {
    ...
    print(oss, "(Q: ", n ? sign * n->GetQ(sign * draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset()) : fpu, ") ", 8, 5);
  };
  auto print_tail = [&](auto* oss, const auto* n) {
    const auto sign = n == node ? -1 : 1;
    std::optional<float> v;
    if (n && n->IsTerminal()) {
      v = n->GetQ(sign * draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
    } else {
  ...
  const auto m_evaluator = network_->GetCapabilities().has_mlh()
                               ...
                               : MEvaluator();
  for (const auto& edge : edges) {
    float Q = edge.GetQ(fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
  ...
  std::partial_sort(
      ...
      [draw_score, this](const auto& a, const auto& b) {
        ...
        if (a_rank == kNonTerminal) {
          ...
          // default isn't used.
          if (a.GetQ(0.0f, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset()) != b.GetQ(0.0f, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset())) {
            return a.GetQ(0.0f, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset()) > b.GetQ(0.0f, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
          }
...
EdgeAndNode Search::GetBestRootChildWithTemperature(float temperature) const {
  ...
  float max_eval = -1.0f;
  const float fpu =
      GetFpu(params_, root_node_, /* is_root= */ true, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
  ...
  for (auto& edge : root_node_->Edges()) {
    ...
    if (edge.GetN() + offset > max_n) {
      ...
      max_eval = edge.GetQ(fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
    }
  }
  ...
  for (auto& edge : root_node_->Edges()) {
    ...
    if (edge.GetQ(fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset()) < min_eval) continue;
    sum += std::pow(
        std::max(0.0f,
                 (max_n <= 0.0f
                      ? edge.GetP()
...
void Search::PopulateCommonIterationStats(IterationStats* stats) {
  ...
  if (root_node_->GetN() > 0) {
    ...
    const float fpu =
        GetFpu(params_, root_node_, /* is_root_node */ true, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
    ...
    for (const auto& edge : root_node_->Edges()) {
      ...
      const auto q = edge.GetQ(fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
  ...
  while (current_path.size() > 0) {
    ...
    if (current_path.back() == -1) {
      ...
      for (Node* child : node->VisitedNodes()) {
        ...
        float q = child->GetQ(draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
        current_util[index] = q + m_evaluator.GetM(child, q);
      }
      const float fpu =
          GetFpu(params_, node, is_root_node, draw_score, visited_pol, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
      for (int i = 0; i < max_needed; i++) {
...
int SearchWorker::PrefetchIntoCache(Node* node, int budget, bool is_odd_depth) {
  ...
  const float puct_mult =
      cpuct * std::sqrt(std::max(node->GetChildrenVisits(), 1u));
  const float fpu =
      GetFpu(params_, node, node == search_->root_node_, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
  for (auto& edge : node->Edges()) {
    if (edge.GetP() == 0.0f) continue;
    // Flip the sign of a score to be able to easily sort.
    // TODO: should this use logit_q if set??
    scores.emplace_back(-edge.GetU(puct_mult) - edge.GetQ(fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset()),
                        edge);
  }
  ...
  for (size_t i = 0; i < scores.size(); ++i) {
    ...
    if (i != scores.size() - 1) {
      ...
      const float q = edge.GetQ(-fpu, draw_score, params_.GetDrawFrustration(), params_.GetDrawDisappointment(), params_.GetScoreUpset());
      if (next_score > q) {
        budget_to_spend =
